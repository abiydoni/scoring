            ...
            <div class="athlete-card bg-slate-800/30 border border-slate-800/80 p-4 rounded-3xl hover:border-slate-700 transition-all flex items-center justify-between group" data-name="<?= esc(strtolower($person['nama'])) ?>">
                <div class="flex items-center gap-3">
                    <?php if (!empty($person['foto'])): ?>
                        <img src="/uploads/anggota/<?= esc($person['foto']) ?>" onclick="showImageModal('/uploads/anggota/<?= esc($person['foto']) ?>')" class="w-10 h-10 rounded-2xl object-cover border border-brand-500/20 shadow-sm shrink-0 cursor-pointer hover:scale-105 transition-transform">
                    <?php else: ?>
                    ...
                    <?php endif; ?>
                    <div onclick="showAthleteStats(event, <?= $person['id'] ?>, 'panahan')" class="cursor-pointer group-hover:opacity-80 transition-opacity">
                        <h4 class="text-sm font-bold text-white hover:text-brand-400 transition-colors"><?= esc($person['nama']) ?></h4>
